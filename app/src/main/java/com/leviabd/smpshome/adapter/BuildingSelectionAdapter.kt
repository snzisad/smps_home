...
import com.leviabd.smpshome.bottom_sheet.ApartmentSelectionActivity
import com.leviabd.smpshome.R
import com.leviabd.smpshome.model.Buildings

class BuildingSelectionAdapter (val activity: Activity, val data: ArrayList<Buildings>): RecyclerView.Adapter<RecyclerView.ViewHolder>() {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return BuildingListHolderClass(
            LayoutInflater.from(activity).inflate(R.layout.layout_building_list, parent, false)
        )
    }
    ...
    override fun onBindViewHolder(headerClass: RecyclerView.ViewHolder, position: Int) {
        if (headerClass is BuildingListHolderClass) {
            ...
            headerClass.cv_building_list.setOnClickListener {
                activity.startActivity(Intent(activity, ApartmentSelectionActivity::class.java))
            }
        }
